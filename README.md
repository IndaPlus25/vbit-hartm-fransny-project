...
3. Oslagbart ekosystem (Pandas & Matplotlib): Pythons bibliotek för datamanipulation (pandas) och grafritning (matplotlib / plotly) är branschstandard. Att aggregera 440 trades, beräkna kumulativ vinst (Cumulative PnL) och rita upp en professionell graf tar bokstavligen 5 rader kod i Python. Att försöka bygga motsvarande grafer direkt i Go hade inneburit onödigt hög komplexitet.
...
* `engine/`: Hjärtat i systemet. Innehåller en **Tillståndsmaskin (State Machine)** som itererar över data ($\mathcal{O}(N)$). Den håller koll på öppna positioner i minnet och loggar en komplett trade först när både *entry* och *exit* är slutförda. Stöder både långa (köp) och korta (blankning) positioner.
